    ...
    for page in range(0, 1):
        ...
        for item in soup.find_all('div', {'class': 'col-xs-2-4 shopee-search-item-result__item'}):
            ...
            if price is not None:
                price = price.find('span', {'class': 'ZEgDH9'}).text.strip().replace(".", ",")
            else:
                price = ''

            location = item.find('div', {'class': 'zGGwiV'})
            if location is not None:
              location = location.text.strip()
            else:
              location = ''
            
            link = item.find('a', {'data-sqe':'link'})
            if link is not None:
              link = 'https://shopee.co.id{}'.format(link.get('href'))
            else:
              link = ''
